...
- (NSDictionary *)getDiagnostics
{
	...
	if (reply) {
		...
		[self setLastError:@"Reply did not contain Status entry"];
	}
	return nil;
}
...
{
	AMIOSDiagnosticsRelay *result = nil;
	if ([self deviceConnect]) {
		if ([self startSession]) {
			result = [[AMIOSDiagnosticsRelay alloc] initWithAMDevice:self];
			[self stopSession];
		}
		[self deviceDisconnect];
	}
...
{
	AMMobileDiagnosticsRelay *result = nil;
	if ([self deviceConnect]) {
		if ([self startSession]) {
			result = [[AMMobileDiagnosticsRelay alloc] initWithAMDevice:self];
			[self stopSession];
		}
		[self deviceDisconnect];
	}
...
{
	AMScreenshotService *result = nil;
	if ([self deviceConnect]) {
		if ([self startSession]) {
			result = [[AMScreenshotService alloc] initWithAMDevice:self];
			[self stopSession];
		}
		[self deviceDisconnect];
	}
